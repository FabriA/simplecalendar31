...
// 		$params  = $item->params;
		...
		switch (strtolower($type))
		{
			...
			case 'email':
				$link = JRoute::_('index.php?option=com_mailto&tmpl=component&link=' . MailToHelper::addLink(SCOutput::_getUriString()));
...
// 				break;
	...
	/**
	 * Retrieves the component name from the xml manifest
	 *
	 * @return mixed
	 */
	public static function getComponentName()
	{
		$xmlFile = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_simplecalendar' . DS . 'com_simplecalendar.xml';
	...
	/**
	 * Retrieves the component version from the xml manifest
	 *
	 * @return mixed
	 */
	...
	/**
	 * Retrieves the component creation date from the xml manifest
	 *
	 * @return mixed
	 */
	public static function getComponentDate()
	{
		$xmlFile = JPATH_ADMINISTRATOR . DS . 'components' . DS . 'com_simplecalendar' . DS . 'com_simplecalendar.xml';
		$data    = JApplicationHelper::parseXMLInstallFile($xmlFile);
		$version = $data['creationDate'];
	 ...
	 *
	 * @return boolean true if it is in development (alpha or beta), false otherwise
	 */
	...
	function _getFileSize($size)
	{
		...
		{
			if ((int) $size >= 1024 && (int) $size < 1048576)
			{
...
// 				break;
			...
			case 'name':
				...
				if ($params && intval($params->get('crop_title_length', '')) > 0)
				{
					$array->name = substr($array->name, 0, intval($params->get('crop_title_length'))) . '...';
				}
