...
class SimplecalendarControllerEvents extends JControllerAdmin
{
	...
	public function export()
	{
		...
		if (sizeof($ids) > 0)
		{
			...
			$entries = $model->getItemsForExport('*', $ids);

			// create a file pointer connected to the output stream
			$output = fopen('php://output', 'w');

			// Use correct encoding (UTF_8)
			fprintf($output, chr(0xEF).chr(0xBB).chr(0xBF));

			fputcsv($output, array_keys($entries[0]), $delimiter);
			...
			header('Content-Disposition: attachment; filename="scexport.csv";');
			exit;
		}
		else
		{
			JError::raiseWarning(500, $model->getError());
			$this->setRedirect('index.php?option=com_simplecalendar&view=events', 'COM_SIMPLECALENDAR_');
		}
	}
}
