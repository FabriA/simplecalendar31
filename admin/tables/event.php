...
class SimplecalendarTableEvent extends JTable
{
	...
	public function check()
	{
		...
		$start_time = JRequest::getVar('jform_start_time');
		...
		$end_time = JRequest::getVar('jform_end_time');
		...
		// Sanitize URL input: add http:// to website URL if available
	...
	public function store($updateNulls = true)
	{
		...
		if (empty($this->id))
		{
			...
			/*if ($this->recur_end_after > 0)
			{
				// TODO $this->id contains the new ID of the saved element
				$firstId    = $this->id;
				$firstAlias = $this->alias;
				$firstName  = $this->name;

				// Reset the id
				$this->id = null;

				for ($i = 1; $i <= (int) $this->recur_end_after; $i++)
				{
					switch ($this->recur_every)
					{
						case '1':
						case '7':
							// Daily + weekly recurrence
							$this->start_dt = date('Y-m-d', strtotime($this->start_dt . '+' . (int) $this->recur_every . ' days'));
							if ($this->end_dt != null || $this->end_dt != '0000-00-00')
							{
								$this->end_dt = date('Y-m-d', strtotime($this->end_dt . '+' . (int) $this->recur_every . ' days'));
							}
							if ($this->reserve_dt != null || $this->reserve_dt != '0000-00-00')
							{
								$this->reserve_dt = date('Y-m-d', strtotime($this->reserve_dt . '+' . (int) $this->recur_every . ' days'));
							}
							break;
						case '30':
							// Monthly recurrence
							$this->start_dt = date('Y-m-d', strtotime($this->start_dt) . '+1 months');
							if ($this->end_dt != null || $this->end_dt != '0000-00-00')
							{
								$this->end_dt = date('Y-m-d', strtotime($this->end_dt) . ' +1 months');
							}
							if ($this->reserve_dt != null || $this->reserve_dt != '0000-00-00')
							{
								$this->reserve_dt = date('Y-m-d', strtotime($this->reserve_dt) . ' +1 months');
							}
							break;
						default:
							JFactory::getApplication()->enqueueMessage(JText::_('SOME_ERROR_OCCURRED'), 'error');
					...
					$this->name  = $firstName . ' (' . $i . ')';
					$this->alias = $firstAlias . '-' . $i;
